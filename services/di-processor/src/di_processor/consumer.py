"""Thin ``raw-dmer-queue`` consumer adapter.
...
application :meth:`Pipeline.run` (Requirements 3.1, 3.5). This adapter is
deliberately thin: it only bridges the consumer's synchronous, dict-based handler
contract to the async, typed pipeline, and owns no business logic.
...
failure (after recording a ``failed`` status), which propagates here and lets the
message dead-letter.
"""
...
def make_handler(pipeline: Pipeline) -> Callable[[dict[str, Any]], None]:
    ...
    validate it into a :class:`RawDmerMessage` and drive the async pipeline to
    ...
    def handle(envelope: dict[str, Any]) -> None:
        message = RawDmerMessage.model_validate(envelope)
...
def make_consumer(
    ...
    ``receiver`` is an Azure ``ServiceBusReceiver`` for ``raw-dmer-queue``;
    ``kwargs`` are forwarded to :class:`ServiceBusConsumer` (e.g. a durable
